...
스프링 설정 정보는 항상 **@Configuration** 을 사용하자!! (@Bean만드로도 스프링등록은 되지만 싱글톤을 지원하진 않는다.)
...
AppConfig라는 클래스를 생성하여 의존관계주입(DI)과 @Bean을 통한 스프링 컨테이너 빈 등록도 관리하였었다.

하지만 스프링의 빈 자동 주입 기능인 **컴포넌트 스캔**을 이용하면 자동으로 빈등록도 가능하다.

* 우리는 하나의 인터페이스와 이를 구현하는 구현객체 하나를 택하여 기능과 구현을 구분하여 설계하였다. 1 interface <=> 1 class 
* 따라서 자동 주입이 가능하다 interface type을 갖는 구현객체가 여러개라면 
* 자동주입에 혼란이 올것이다.
...
> 이제 **사용하는 각 클래스가 컴포넌트 스캔의 대상이 되도록 @Component 애노테이션을 붙여주자**
...
<img src="C:\Users\afrad\OneDrive\문서\SpringFramwork\prototypeScope.PNG" alt="prototypeScope" style="zoom:150%;" />
